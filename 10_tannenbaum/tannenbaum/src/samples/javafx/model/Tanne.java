...
public class Tanne {
    ...
    private void zeichneKrone() {
        for (int i = 1; i <= this.kronenhoehe; ++i)
            zeichneZeile(this.kronenhoehe - i, 2 * i - 1);
    }

    private void zeichneStamm() {
        for (int i = 1; i <= this.stammhoehe; ++i)
            zeichneZeile(this.kronenhoehe - this.stammbreite / 2 - 1, this.stammbreite);
    }
